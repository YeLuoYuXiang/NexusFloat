...
![License](https://img.shields.io/badge/License-GPLv3.0-337ab7.svg)
...
GPU 数据需要 root，不给 root 的话其余指标照常工作。
...
这是这个模块花时间最多的地方。FPS 有六个来源，各自一个开关，按下面的顺序依次尝试，
取到就停。数值后面会跟一个来源字母（诊断模式），某个来源在这台机器上不准时，
按字母关掉对应的那个就行。
...
1. Hook 折叠状态栏 Fragment 的 `onViewCreated`，能直接拿到状态栏的 View 树。
   类名在 Android 各版本里改过多次，所以维护了一张候选表逐个试。
2. Hook `Instrumentation.callApplicationOnCreate` 和 `Application.attach`，
...
1. RemotePreferences（LSPosed 提供的跨进程 prefs）
2. ContentProvider 快照——App 侧把设置写进一个 exported Provider，SystemUI 直接查
3. `Settings.Global` 里的 Base64 blob——前两条都不通时由 root 写入
...
见 [CONTRIBUTING.md](CONTRIBUTING.md)。提 FPS 问题时请附上诊断字母。
