 ...
 *
 * 为什么要它：GPU 频率/占用是模块 App 进程用常驻 su 读出来、再写进 prefs 的，
 * SystemUI 只是读的一方。监视条一隐藏，SystemUI 这边的采集线程会随 View detach 停掉，
 * 但模块进程那个 su 读取循环在另一个进程里，丝毫不受影响，会一直每秒读一遍 sysfs。
...
public final class CollectorSignal {
     ...
     * adb am force-stop），这种状态下系统只放行带 FLAG_INCLUDE_STOPPED_PACKAGES
     * 的广播——普通广播、ContentProvider query 都会被直接拒绝。
     * 划卡只是普通杀进程、不置 stopped，所以划卡没事，只有全部清除后才需要它。
     *
     * 这个 flag 由发送方加，而发送方就是被注入的 SystemUI 进程，所以不用 hook
     * 系统框架、也不用把模块作用域扩到 system。
     */
    public static void wakeModule(Context context) {
        if (context == null) {
            return;
        }
        try {
            Intent intent = new Intent(Constants.Component.ACTION_WAKE);
            // 显式指定收件包：只有我们自己的接收者会收到
            intent.setPackage(Constants.Package.MODULE);
            intent.addFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
            context.sendBroadcast(intent);
        } catch (Throwable t) {
            LogUtils.w("CollectorSignal wake failed", t);
        }
    }
